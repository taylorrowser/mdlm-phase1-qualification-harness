...
function observationUsable(observation) {
  return observation.signal === null && !observation.timedOut && !observation.spawnError &&
    observation.cleanupComplete && !observation.stdoutTruncated && !observation.stderrTruncated;
}
...
export async function qualify(configPath) {
  ...
  const defaults = { env: nodeChildEnvironment(), deadlineMs: config.deadlineMs, termGraceMs: config.termGraceMs };
  const probes = [];
  ...
  for (const control of config.negativeControls ?? []) {
    ...
    negativeControls.push({
      ...
      expected,
      observation,
      discriminated: observationUsable(observation) && !expectedMatches(observation, expected),
    });
  }
  ...
  return {
    schemaVersion: 1,
    kind: 'phase1-environment-qualification',
