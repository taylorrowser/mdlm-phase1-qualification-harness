...
function matches(observation, expected) {
  return observation.status === expected.status && observation.signal === null &&
    ...
    observation.streams?.complete === true && !observation.stdoutTruncated && !observation.stderrTruncated &&
...
function validateLiteralProfile(profile) {
  ...
  if (profile.executionProfile !== undefined &&
      (typeof profile.executionProfile.id !== 'string' || profile.executionProfile.id.length === 0)) {
    throw new Error('controlled pilot executionProfile must declare an identity-bearing id');
  }
  if (profile.identities !== undefined) throw new Error('controlled pilot profiles must not supply unauthenticated identities');
  ...
  try {
    ...
    if (profile.executionProfile !== undefined) {
      ...
      return {
        ...
        capabilities: profile.capabilities,
        ...(profile.qualificationStatus === undefined ? {} : { qualificationStatus: profile.qualificationStatus }),
